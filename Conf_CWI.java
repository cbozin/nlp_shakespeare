...
public class Conf
{
	...
	{
		// IntWritable object to store the count
		private IntWritable result = new IntWritable();

		// Reduce function: sums up word counts and emits the result
		@Override
		public void reduce(Text key, Iterable<IntWritable> values, Context context) throws IOException, InterruptedException
		{
			...
			result.set(sum);
			context.write(key, result); // key, count(key)
		}
	}
		...
		public void map(Object key, Text value, Context context) throws IOException, InterruptedException
		{
			...
			while (lineItr.hasMoreTokens())
			{
				...
				if (word.contains(":"))
				{
					...
					k.set(first);
					v.set(second + "|" + count);
					context.write(k, v); // x, y|count(x,y)

					k.set(second);
					v.set(first + "|" + count);
					context.write(k, v); // y, x|count(x,y)
				}
				else
				{
					k.set(word);
					v.set(count);
					context.write(k, v); // x, count(x)
				}
			}
		}
	}
		...
		public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException
		{
			...
			for (Text val : values)
			{
				...
				if (s.contains("|"))
				{
					...
					k.set(key.toString() + ":" + word);
					num /= count;
					v.set("" + num);
					context.write(k, v); // x:y, count(x,y)/count(x)
				}
			}
		}
	}
